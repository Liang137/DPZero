...
def forward_wrap_with_option_len_dpzero(self, input_ids=None, labels=None, option_len=None, num_options=None, return_dict=None, dpzero=False, **kwargs):
    """
    ...
    if num_options is not None: 
        ...
        if dpzero:
            loss_unred = loss_fct(shift_logits.transpose(1,2), shift_labels)
            mask = (shift_labels != -100)
            loss = (loss_unred * mask).sum(dim=1) / mask.sum(dim=1)
        else:
